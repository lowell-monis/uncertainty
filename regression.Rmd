...
set.seed(402)
x_full <- 0:10
y_full <- 5 + 2 * x_full + rnorm(length(x_full), sd = 2)
df_full <- data.frame(x = x_full, y = y_full)
df_train <- subset(df_full, x >= 2 & x <= 8)

ggplot(df_full, aes(x, y)) +
  geom_point(color = "gray", size = 2) +
  geom_point(data = df_train, aes(x, y), color = "blue", size = 2) +
  geom_smooth(data = df_train, aes(x, y), method = "lm", se = FALSE, color = "red") +
  geom_vline(xintercept = c(2, 8), linetype = "dashed", color = "black") +
  annotate("text", x = 1, y = 25, label = "Extrapolation", angle = 90, vjust = -0.5, color = "darkred") +
  annotate("text", x = 9, y = 25, label = "Extrapolation", angle = 90, vjust = -0.5, color = "darkred") +
  annotate("text", x = 5, y = 25, label = "Interpolation", color = "darkgreen") +
  labs(
    x = "Predictor (x)",
    y = "Outcome (y)",
    caption = "Figure: Linear model trained on x = 2 to 8. Predictions within this range are interpolations; outside are extrapolations."
  ) +
  theme_minimal() +
  theme(plot.caption = element_text(hjust = 0.5, face = "bold", size = 11))
