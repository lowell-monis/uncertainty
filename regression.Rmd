...
```
```{r echo=FALSE, message=FALSE, warning=FALSE, fig.align='center'}
df_train_partial <- subset(df_full, x >= 0 & x <= 100)
model_partial <- lm(y ~ x, data = df_train_partial)

model_full <- lm(y ~ x, data = df_full)

df_preds <- data.frame(x = seq(0, 150, length.out = 500))
df_preds$y_partial <- predict(model_partial, newdata = df_preds)
df_preds$y_full <- predict(model_full, newdata = df_preds)

ggplot(df_full, aes(x, y)) +
  geom_point(color = "gray", size = 2, alpha = 0.5) +
  geom_line(data = df_preds, aes(x = x, y = y_partial), color = "darkgreen", linewidth = 1.2) +
  geom_line(data = df_preds, aes(x = x, y = y_full), color = "blue", linewidth = 1.2, linetype = "dashed") +
  annotate("text", x = 90, y = 15, label = "Partial Fit (0–100)", color = "darkgreen", hjust = 0) +
  annotate("text", x = -9, y = max(df_preds$y_full), label = "Full Fit (-10–110)", color = "blue", hjust = 0) +
  labs(
    x = "Predictor (x)",
    y = "Outcome (y)",
    title = "Comparing Linear Models Trained on Different Data Ranges",
    subtitle = "Solid green: trained on x = 0–100 | Dashed blue: trained on x = -10–110"
  ) +
  reg_theme
```
